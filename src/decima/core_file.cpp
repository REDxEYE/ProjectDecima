...
void Decima::CoreFile::unpack() {
    ...
    storage.resize(uncompressed_size);
    {
        ...
        for (auto chunk_entry = chunk_entry_begin; chunk_entry != chunk_entry_end; ++chunk_entry) {
            ...
            memcpy(data_buffer.data(), filebuffer->data() + chunk_entry->compressed_offset, chunk_entry->compressed_size);
            //decrypt
            if (encrypted)
...
std::pair<std::vector<Decima::ChunkEntry>::iterator, std::vector<Decima::ChunkEntry>::iterator> Decima::CoreFile::get_chunk_boundaries() {
