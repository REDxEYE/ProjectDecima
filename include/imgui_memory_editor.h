...
//        const char* format_address = OptUpperCaseHex ? "%0*" _PRISizeT "X: " : "%0*" _PRISizeT "x: ";
        const char* format_address = OptUpperCaseHex ? "%0*" _PRISizeT "i: " : "%0*" _PRISizeT "i: ";
        const char* format_data = OptUpperCaseHex ? "%0*" _PRISizeT "X" : "%0*" _PRISizeT "x";
//        const char* format_range = OptUpperCaseHex ? "Range %0*" _PRISizeT "X..%0*" _PRISizeT "X" : "Range %0*" _PRISizeT "x..%0*" _PRISizeT "x";
        const char* format_range = OptUpperCaseHex ? "Range %0*" _PRISizeT "i..%0*" _PRISizeT "i"
                                                   : "Range %0*" _PRISizeT "i..%0*" _PRISizeT "i";
            ...
            for (int n = 0; n < Cols && addr < mem_size; n++, addr++) {
                ...
                if (DataEditingAddr == addr) {
                    ...
                    if (DataEditingTakeFocus) {
                        ...
                        sprintf(AddrInputBuf, format_address, s.AddrDigitsCount, base_display_addr + addr);
        ...
        if (OptShowOptions) {
            ...
            if (ImGui::InputText("##addr", AddrInputBuf, 32, ImGuiInputTextFlags_EnterReturnsTrue)) {
                size_t goto_addr;
                if (sscanf(AddrInputBuf, "%" _PRISizeT "i", &goto_addr) == 1) {
            ...
            if (GotoAddr != (size_t) -1) {
                if (GotoAddr < mem_size) {
                    ...
                    ImGui::SetScrollFromPosY(
                            ImGui::GetCursorStartPos().y + (GotoAddr / Cols) * ImGui::GetTextLineHeight());
                    ImGui::EndChild();
        ...
        if (OptShowDataPreview) {
            ...
            if (has_value)
                DisplayPreviewData(DataPreviewAddr, mem_data, mem_size, PreviewDataType, DataFormat_Dec, buf,
                                   (size_t) IM_ARRAYSIZE(buf));
            ImGui::Text("Dec");
            ImGui::SameLine(x);
            ImGui::TextUnformatted(has_value ? buf : "N/A");
            if (has_value)
                DisplayPreviewData(DataPreviewAddr, mem_data, mem_size, PreviewDataType, DataFormat_Hex, buf,
                                   (size_t) IM_ARRAYSIZE(buf));
            ImGui::Text("Hex");
            ImGui::SameLine(x);
            ImGui::TextUnformatted(has_value ? buf : "N/A");
            if (has_value)
                DisplayPreviewData(DataPreviewAddr, mem_data, mem_size, PreviewDataType, DataFormat_Bin, buf,
                                   (size_t) IM_ARRAYSIZE(buf));
            ImGui::Text("Bin");
            ImGui::SameLine(x);
            ImGui::TextUnformatted(has_value ? buf : "N/A");
    ...
    void DisplayPreviewData(size_t addr, const ImU8* mem_data, size_t mem_size, ImGuiDataType data_type,
